...
class EnhancedMainNetwork(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_dim: int = 512, hidden_dim: int = 2048, 
                 num_layers: int = 12, num_heads: int = 8, max_seq_len: int = 4096):
        super().__init__()
        self.vocab_size = vocab_size
        self.embed_dim = embed_dim
        self.max_seq_len = max_seq_len
        ...
        # Добавленные недостающие компоненты
        self.semantic_projection = nn.Linear(embed_dim, embed_dim)
        self.topic_classifier = nn.Linear(embed_dim, 100)  # Начальное количество тем
        self.confidence_head = nn.Linear(embed_dim, 1)
...
class TopicBasedAISystem:
    ...
    def __init__(self, storage_path: str, model_name: str = "gpt2"):
        """Инициализация продвинутой системы ИИ"""
        ...
        # Исправлено: уменьшены размеры для экономии памяти
        ...
        print(f"Система ИИ инициализирована:")
