...
class SemanticWeightMerger:
    ...
    def merge_transformer_layers(base_layers: nn.ModuleList, cluster_layers_dict: Dict[str, nn.ModuleList]) -> nn.ModuleList:
        """Объединяет трансформер слои путем расширения размерностей"""
        ...
        for layer_idx, base_layer in enumerate(base_layers):
            ...
            # Создаем расширенный слой
            merged_layer = SemanticWeightMerger._merge_single_transformer_layer(base_layer, cluster_layers)
    ...
    def merge_embeddings(base_embedding: nn.Embedding, cluster_embeddings: Dict[str, nn.Embedding]) -> nn.Embedding:
        """Объединяет эмбеддинги путем расширения размерности"""
        if not cluster_embeddings:
            return base_embedding
        
        base_embed_dim = base_embedding.embedding_dim
        total_embed_dim = base_embed_dim * (1 + len(cluster_embeddings))
        
        merged_embedding = nn.Embedding(base_embedding.num_embeddings, total_embed_dim)
        
        with torch.no_grad():
            # Копируем базовые эмбеддинги
            merged_embedding.weight[:, :base_embed_dim] = base_embedding.weight
            
            # Добавляем эмбеддинги кластеров
            offset = base_embed_dim
            for topic, cluster_emb in cluster_embeddings.items():
                merged_embedding.weight[:, offset:offset+base_embed_dim] = cluster_emb.weight
                offset += base_embed_dim
...
class EnhancedRouterNetwork(nn.Module):
    """Улучшенная нейросеть-маршрутизатор с семантическим пониманием"""
    
    def __init__(self, vocab_size: int, embed_dim: int = 768, hidden_dim: int = 2048, 
                 num_layers: int = 12, num_heads: int = 12, max_seq_len: int = 4096,
                 max_topics: int = 100000):
        super().__init__()
        ...
        # Многоуровневые трансформер блоки для глубокого понимания
        ...
        # Многоголовая система классификации тем
        ...
        self.topic_classifier = nn.Linear(embed_dim, max_topics)
        
        # Система уверенности и релевантности
        self.confidence_head = nn.Linear(embed_dim, 1)
        self.relevance_head = nn.Linear(embed_dim, max_topics)
        
        # Кластеры по языкам (динамически расширяемые)
        self.language_clusters = nn.ModuleDict()
        self.topic_embeddings = nn.Embedding(max_topics, embed_dim)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
                topic_candidates: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        batch_size, seq_len = input_ids.shape
        device = input_ids.device
        
        # Эмбеддинги с позиционным кодированием
        ...
        # Применяем трансформер блоки
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, src_key_padding_mask=~attention_mask.bool())
        
        # Семантическое пулинг с вниманием
        semantic_repr = self.semantic_projection(hidden_states)
        
        # Взвешенное пулинг по важности токенов
        ...
        # Многоуровневая классификация тем
        ...
        # Комбинированные скоры для более точного выбора
    ...
    def merge_language_clusters(self, active_languages: List[str], 
        ...
        """Объединяет кластеры языков в единую сеть"""
        if not language_weights:
            return
        
        # Создаем временные кластеры для активных языков
        cluster_networks = {}
        for lang in active_languages:
            if lang in language_weights:
                cluster_networks[lang] = self._create_language_cluster(language_weights[lang])
        
        # Объединяем с базовой сетью
        if cluster_networks:
            merger = SemanticWeightMerger()
            
            # Объединяем эмбеддинги
            cluster_embeddings = {lang: net.token_embedding for lang, net in cluster_networks.items()}
            self.token_embedding = merger.merge_embeddings(self.token_embedding, cluster_embeddings)
            
            # Объединяем трансформер блоки
            cluster_blocks = {lang: net.transformer_blocks for lang, net in cluster_networks.items()}
            self.transformer_blocks = merger.merge_transformer_layers(self.transformer_blocks, cluster_blocks)
...
class EnhancedMainNetwork(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_dim: int = 2048, hidden_dim: int = 8192, 
                 num_layers: int = 32, num_heads: int = 32, max_seq_len: int = 200000):
        super().__init__()
        self.vocab_size = vocab_size
        self.embed_dim = embed_dim
        self.max_seq_len = max_seq_len
        ...
        # Трансформер блоки с улучшенной архитектурой
        self.transformer_blocks = nn.ModuleList([
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
        ...
        # Создаем каузальную маску для автрегрессивной генерации
        ...
        for i, block in enumerate(self.transformer_blocks):
            # Применяем адаптеры тем если они есть
    ...
    def merge_topic_clusters_advanced(self, topic_weights: Dict[str, Dict[str, torch.Tensor]], 
        ...
        """Продвинутое объединение кластеров тем с сохранением знаний"""
        ...
        # Создаем кластеры для каждой темы
        topic_networks = {}
        for topic, weights in topic_weights.items():
            topic_networks[topic] = self._create_topic_network(weights, semantic_info.get(topic, {}))
        
        # Используем семантический объединитель
        merger = SemanticWeightMerger()
        
        # Объединяем эмбеддинги
        cluster_embeddings = {}
        for topic, network in topic_networks.items():
            cluster_embeddings[topic] = network['token_embedding']
        
        if cluster_embeddings:
            self.token_embedding = merger.merge_embeddings(self.token_embedding, cluster_embeddings)
        
        # Объединяем трансформер блоки
        cluster_transformer_blocks = {}
        for topic, network in topic_networks.items():
            cluster_transformer_blocks[topic] = network['transformer_blocks']
        
        if cluster_transformer_blocks:
            self.transformer_blocks = merger.merge_transformer_layers(
                self.transformer_blocks, cluster_transformer_blocks
            )
        
        # Создаем адаптеры для тонкой настройки
        self._create_topic_adapters(topic_networks, semantic_info)
...
class TopicBasedAISystem:
    """Основная система ИИ с полным соответствием техническому заданию"""
    
    def __init__(self, storage_path: str = "./advanced_ai_storage", 
                 model_name: str = "microsoft/DialoGPT-medium"):
        self.storage_path = Path(storage_path)
        self.storage_path.mkdir(exist_ok=True, parents=True)
        
        print("Инициализация продвинутой ИИ системы...")
        ...
        try:
            ...
            print(f"Ошибка загрузки токенизатора {model_name}: {e}")
            print("Используем базовый токенизатор gpt2")
        ...
        self.weight_storage = AdvancedCompressedWeightStorage(str(self.storage_path / "weights"))
        
        # Инициализация нейросетей
        self.router = EnhancedRouterNetwork(self.vocab_size)
        self.main_network = EnhancedMainNetwork(self.vocab_size)
        ...
        # Кэш активных компонентов
        self.active_topics = set()
        self.active_languages = set()
        self.performance_metrics = defaultdict(list)
        
        print(f"Система инициализирована. Тем: {len(self.topic_registry)}, Языков: {len(self.language_registry)}")
    ...
    def add_topic_advanced(self, topic: str, description: str = "", 
        ...
        """Добавляет новую тему с расширенными метаданными"""
        if topic not in self.topic_registry:
            ...
            # Обновляем связи с родительскими темами
    ...
    def route_request_advanced(self, user_input: str, 
        ...
        with torch.no_grad():
            ...
            top_values, top_indices = torch.topk(
                topic_probs, 
                k=min(max_topics, len(self.topic_registry)), 
                dim=-1
            )
    ...
    def _generate_text_advanced(self, input_ids: torch.Tensor, attention_mask: torch.Tensor,
        ...
        for step in range(max_length):
            ...
            current_mask = torch.ones(current_length, device=generated.device).unsqueeze(0)
            ...
            with torch.no_grad():
                ...
                if past_tokens:
                    for token_id in set(past_tokens[-50:]):  # Учитываем последние 50 токенов
    ...
    def train_router_advanced(self, training_data: List[Dict[str, Any]], 
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(processed_data), batch_size):
                ...
                batch_topic_labels = torch.zeros((len(batch_data), self.router.max_topics))
                
                for idx, item in enumerate(batch_data):
                    for topic in item['topics']:
                        if topic in self.topic_registry:
                            topic_id = self.topic_registry[topic]['id']
                            if topic_id < self.router.max_topics:
    ...
    def _partition_weights_by_language(self, weights: Dict[str, torch.Tensor], 
        ...
        """Семантически разделяет веса по языкам"""
        ...
        for name, weight in weights.items():
            if len(weight.shape) >= 2:
                ...
                partition_size = weight.shape[0] // total_languages
                start_idx = language_id * partition_size
                end_idx = start_idx + partition_size if language_id < total_languages - 1 else weight.shape[0]
                
                partitioned_weights[name] = weight[start_idx:end_idx].clone()
            else:
                # Для одномерных - копируем с весовым коэффициентом
    ...
    def train_main_network_advanced(self, training_data: List[Dict[str, Any]], 
        ...
        total_steps = (len(main_data) // (batch_size * gradient_accumulation)) * epochs
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(all_samples), batch_size):
                ...
                if (batch_count + 1) % gradient_accumulation == 0:
            ...
            avg_loss = epoch_loss / max(batch_count // gradient_accumulation, 1)
            training_losses.append(avg_loss)
            
            print(f"Эпоха {epoch+1}/{epochs}, Потеря: {avg_loss:.6f}")
    ...
    def _process_dialog_sample(self, data_point: Dict[str, Any], topic: str) -> Optional[torch.Tensor]:
        ...
        # Создаем маски для обучения только на ответах ассистента
        targets = input_ids.clone()
        targets[:, :-1] = input_ids[:, 1:]
    ...
    def _partition_weights_by_topic(self, weights: Dict[str, torch.Tensor], 
        ...
        """Семантически разделяет веса по темам"""
        topic_id = self.topic_registry[topic]['id']
        ...
        for name, weight in weights.items():
            if 'embedding' in name.lower():
                # Эмбеддинги - сохраняем с семантическим весом
                partitioned_weights[name] = weight * semantic_weight
            
            elif 'transformer_blocks' in name and len(weight.shape) >= 2:
                # Трансформер блоки - семантическое разделение
                # Извлекаем слои, наиболее релевантные теме
                layers_per_topic = max(1, weight.shape[0] // total_topics)
                start_idx = topic_id * layers_per_topic
                end_idx = min(start_idx + layers_per_topic, weight.shape[0])
                
                if start_idx < weight.shape[0]:
                    partitioned_weights[name] = weight[start_idx:end_idx].clone()
                else:
                    # Если выходим за границы, берем последние слои
                    partitioned_weights[name] = weight[-layers_per_topic:].clone()
            
            elif len(weight.shape) >= 2:
                # Другие двумерные веса - интеллектуальное разделение
                partition_size = max(1, weight.shape[0] // total_topics)
                start_idx = topic_id * partition_size
                end_idx = min(start_idx + partition_size, weight.shape[0])
                
                partitioned_weights[name] = weight[start_idx:end_idx].clone()
            
            else:
                # Одномерные веса - копируем с коэффициентом
